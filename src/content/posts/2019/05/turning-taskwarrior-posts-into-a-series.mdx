...
categories:
  - post
cover_image: /img/2019/cover-turning-taskwarrior-posts-into-a-series.jpg
date: 2019-05-12 00:00:00-07:00
description: There's a new taxonomy for posts that are written in a particular order!
posse:
  - https://hackers.town/@randomgeek/102082058493417205
tags:
  - ssg
uses:
  - hugo
...
[Taskwarrior posts][tw-posts], which specifically build on each
other.

[tw-posts]: /uses/taskwarrior/

[Hugo](https://gohugo.io) supports series as a
...
![Listing of all series, currently just Taskwarrior Babysteps](@img/2019/series-listing.png)

What if I follow [taskwarrior-babysteps](/series/taskwarrior-babysteps)
link?

![The Taskwarrior Babysteps series listing](@img/2019/taskwarrior-series-initial.png)
...
```text title="layouts/series/series.html"
{{ define "main" }}
  <h1>{{ .Title }}</h1>
  {{- .Content -}}
  {{ partial "term-path" . }}
...
```markdown title="content/series/taskwarrior-babysteps/_index.md"
---
...
![The Taskwarrior Babysteps series after tuning](@img/2019/taskwarrior-series-adjusted.png)
...
```text title="layouts/partials/meta.html"
⋮
{{ range .Params.series }}
...
```text title="layouts/partials/series-details.html"
{{- if .Params.series -}}
...
![Post header with series information, and a little styling to make it stand out](@img/2019/taskwarrior-series-post-header.png)
