...
or less the same form since 2001. There's better stuff out there now.
:::
...
Okay, there is also [Jython][jython] and [IronPython][iron-python], but both of these are for special platforms -
[Java][java] and [.NET][dotnet], respectively.
As such, they have more specific requirements than I am interested in covering here.
Don’t let that stop *you* from feeling bold, though!
...
:::note
PythonWin is still available to you as part of the [win32all][pywin32] package,
but you need to download and install it separately from your Python distribution.
ActiveState Python for Linux includes IDLE, so I will be getting my Python from them for this article.
:::
...
Your distribution probably has some version of Python available.
I prefer to get mine from [ActiveState][active-state].
It’s easy enough to install, and is usually more up-to-date than what ships with most Linux distributions.
There are also fewer concerns about figuring out whether you installed the `idle` or `pydoc` packages.
You just install [ActivePython][active-python] and get to work.

[active-state]: https://activestate.com
[active-python]: https://www.activestate.com/platform/supported-languages/python/
...
Find links to download the latest version of Python at the official [Python downloads page][python-downloads].
...
- [How To Think Like a Computer Scientist In Python](http://openbookproject.net/thinkcs/python/english2e/)
