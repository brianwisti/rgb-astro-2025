...
categories:
- post
cover_caption: relevant pictures are overrated; this dog and cat are cute together
cover_image: /img/2022/cover-processing-sass-in-eleventy.jpg
date: 2022-01-10 01:00:00-08:00
description: Specifically, building `main.scss` when `_base.scss` changes
draft: false
posse:
- https://hackers.town/@randomgeek/107595588438140013
- https://twitter.com/brianwisti/status/1480352167336849412
tags:
- eleventy
- scss
- css
...
```javascript title="src/_11ty/handlers/SassHandler.js"
...
```javascript title="Import libraries"
...
```javascript title="Define input and output paths for sass"
...
:::note
Common sense suggests these should be defined in a config file somewhere. I'm
...
```javascript title="Set build time guard variables"
...
```javascript title="Ensure we need to build the CSS"
...
```javascript title="Remember and log this build"
lastSassBuild = now;
...
```javascript title="Compile the Sass and write the CSS"
