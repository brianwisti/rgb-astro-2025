...
- /post/2020/04/h-entry-microformat-for-indieweb-posts/
categories:
- post
cover_caption: looking at the interpreted microformats for a post, in [bat](https://github.com/sharkdp/bat)
cover_image: /img/2020/cover-h-entry-microformat-for-indieweb-posts.png
date: 2020-04-26 13:02:33-07:00
description: In which I go overboard with Hugo and Python for a quick Sunday task
draft: false
posse:
- https://hackers.town/@randomgeek/104066683729018365
- https://twitter.com/brianwisti/status/1254502662453833728
tags:
- indieweb
- microformats
- hugo
...
:::note{title="But I want to try Webmentions!"}
...
walk you through the process. [IndieWebify.my](https://IndieWebify.me) in particular
...
well expose some of the Hugo templating as well.
...
Python][weighing-files].

[weighing-files]: /post/2019/06/weighing-files-with-python/

![post header with minimal h-entry info](@img/2020/h-entry-with-title.png)
...
I organize my Hugo content by [type][hugo-type] — currently
[Note][note-category] or [Post][post-category] — and then add optional details
with [categories][category-list] and [tags][tag-list]. The post should probably
show each of those as a `p-category`.

:::note{title="2022-03-27"}
Now the main type is [blog post][blog-section]. Posts and notes are categories
within the blog. All that timestamped stuff goes in one section with this
version of the site. Category and tag are still `p-category` though.
...
![h-entry with categories](@img/2020/h-entry-with-categories.png)
...
![full h-entry](@img/2020/h-entry-full.png)
...
h-entry at IndieWebify or copy and paste to [microformats.io](https://microformats.io),
