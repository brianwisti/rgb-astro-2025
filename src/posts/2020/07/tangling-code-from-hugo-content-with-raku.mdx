...
categories:
- post
cover_caption: You know what else I can tangle? Yarn!
cover_image: /img/2020/cover-tangling-code-from-hugo-content-with-raku.jpg
date: 2020-07-08 21:45:00-07:00
description: I could just use Org mode, but noo that's too easy
draft: false
posse:
- https://hackers.town/@randomgeek/104482085213153476
- https://twitter.com/brianwisti/status/1281087955398991874
tags:
- raku-lang
...
:::note{title="2020-09-03"}
[@codesections@fosstodon.org][codesections] found a typo! I forgot to *show* the target file name once command line arguments are in place. It should go `raku tangle-fragments.raku index.md`.
...
I based `{{</* code */>}}` here on a shortcode from the [Hugo
docs][hugo-docs]. It presents highlighted code with additional context.
...
be entered via <kbd>Control-k</kbd> followed by `<<`.  Or if you've set up a
[Compose][compose] key, it's <kbd>Compose</kbd> followed by `<<` in any editor.
...
I may want to think about a [class][classtut] or something to hold the complexity.

[classtut]: https://docs.raku.org/language/classtut
:::
...
```python title="Generated rich-hello.py"
from rich import print
from rich.panel import Panel
from rich.markdown import Markdown
...
![Formatted output using Rich](@img/2020/rich-panel.png)
...
- rendering fragment names in such a way that syntax highlighters can do something pretty with them, especially when writing code in a language that [Chroma][chroma] has heard of
- hidden blocks
