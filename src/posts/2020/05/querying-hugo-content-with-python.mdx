---
aliases:
- /post/2020/05/querying-hugo-content-with-python/
categories:
- post
cover_caption: |
    My blogging output by year – bet you didn’t know there were blogs in 1 CE
cover_image: /img/2020/cover-querying-hugo-content-with-python.png
date: 2020-05-06 23:37:00-07:00
description: |
    I had entirely too much fun replacing my site metadata script with sqlite-utils
draft: false
posse:
- https://hackers.town/@randomgeek/104125796417600382
- https://twitter.com/brianwisti/status/1258285359064125441
tags:
- hugo
- python
- site
...
:::note{title="2020-11-09"}

Used my [Raku tangles][raku-tangles] to touch up the code and make a downloadable link. I want to tidy it so bad, but I’d need to rewrite large chunks of this post. But hey it’s there and it works. Have fun!
...
[Perl][perl] script he wrote for querying site metadata via an in-memory
...
```python title="Import entry dependencies"
...
```python title="import sqlite-utils"
from sqlite_utils import Database
```

```python title="build databse"
DB_NAME =  "site.db"
...
```python title="define script execution"
...
![lightly styled with custom SQL](@img/2020/datasette-sql.png)
...
![Datasette detail view of a post](@img/2020/datasette-entry.png)

[Plugins][plugins] add all sorts of functionality. [datasette-vega][] draws
graphs of query results.

![Charting interface with entries posted per year](@img/2020/datasette-graph.png)
